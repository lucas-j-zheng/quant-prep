// Pure string-processing core for our Markdown -> HTML pipeline.
//
// This module is intentionally free of React and `katex/contrib/auto-render`
// imports so it can run in a plain Node script (see scripts/check-content.mjs).
// Markdown.jsx imports these helpers; keep ALL pipeline logic here so the app
// and the content guard can never drift out of sync.
//
// Strategy:
...
export function hasProse(inner) {
...
export function findClose(s, from) {
...
export function restoreCode(html, blocks) {
...
export const isTableSep = (line) =>
  /^\s*\|?[\s:|-]*-[\s:|-]*\|?\s*$/.test(line) && line.includes('-')
export const isTableRow = (line) => line.trim().startsWith('|')
